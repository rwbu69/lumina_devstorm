                ...
                <div class="w-full lg:w-64 shrink-0">
                    <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 lg:sticky lg:top-24">
                        <h5 class="font-serif font-bold text-slate-800 text-lg mb-5 flex items-center">
                            <i class="bi bi-filter-left me-2 text-indigo-600 text-xl"></i> Filter Kategori
                        </h5>
                                ...
                                <template x-for="book in results" :key="book.id">
                                    <a :href="'/catalog/' + book.id" class="flex items-center gap-3 p-3 hover:bg-slate-50 border-b border-slate-100 transition-colors">
                                        <div class="w-10 h-14 bg-indigo-50 rounded shadow-sm flex items-center justify-center shrink-0">
                                            <i class="bi bi-book text-indigo-400 text-xl"></i>
                                        </div>
                                        <div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <div class="hidden lg:block w-80" x-data="searchPreview()">
                            <form action="{{ route('catalog.index') }}" method="GET" class="relative">
                                ...
                                <div x-show="isOpen && results.length > 0" 
                                     x-transition.opacity.duration.300ms
                                     class="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 shadow-2xl rounded-xl overflow-hidden z-50">
                                    <div class="max-h-80 overflow-y-auto">
                                        <template x-for="book in results" :key="book.id">
                                            <a :href="'/catalog/' + book.id" class="flex items-center gap-3 p-3 hover:bg-slate-50 border-b border-slate-100 transition-colors">
                                                <div class="w-10 h-14 bg-indigo-50 rounded flex items-center justify-center shrink-0">
                                                    <i class="bi bi-book text-indigo-400 text-lg"></i>
                                                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                        @forelse ($books as $book)
                            ...
                            <div class="group bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200 rounded-2xl overflow-hidden flex flex-col h-full relative">
                                ...
                                <span class="absolute top-3 left-3 z-20 bg-indigo-600/90 backdrop-blur text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-sm">
                                    {{ $book->category->nama }}
                                </span>
                                ...
                                @else
                                    <div class="relative bg-slate-100 h-48 flex items-center justify-center p-4 border-b border-slate-100 overflow-hidden">
                                        ...
                                        <div class="relative z-10 w-24 h-32 bg-white rounded shadow-sm flex flex-col items-center justify-center p-2 border border-slate-200 text-center group-hover:-translate-y-1 transition-transform duration-300">
                                            <i class="bi bi-book-half text-slate-300 text-2xl mb-1"></i>
                                ...
                                <div class="p-4 flex flex-col flex-grow justify-between bg-white">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex gap-2">
                                            <a href="{{ route('catalog.show', $book->id) }}" class="flex-1 flex justify-center items-center py-2 px-3 rounded-lg border border-slate-300 hover:border-slate-400 text-slate-700 bg-white text-sm font-bold transition-colors">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            ...
                                            <div x-data="{
                                                ...
                                                <button @click="addToCart" :disabled="loading" :class="inCart ? 'bg-emerald-50 hover:bg-emerald-100 border-emerald-200 text-emerald-700' : 'bg-indigo-600 hover:bg-indigo-700 border-transparent text-white shadow-sm'" class="flex justify-center items-center py-2 px-3 rounded-lg border text-sm font-bold transition-colors w-full h-full disabled:opacity-75 disabled:cursor-not-allowed">
                                                    <template x-if="loading">
                                                        <span><i class="bi bi-arrow-repeat animate-spin me-2 inline-block"></i> Proses...</span>
                                                    </template>
                                                    <template x-if="!loading && inCart">
                                                        <span><i class="bi bi-check-circle-fill me-2"></i> Di Keranjang</span>
                                                    </template>
                                                    <template x-if="!loading && !inCart">
                                                        <span><i class="bi bi-cart-plus me-2"></i> Tambah</span>
                                                    </template>
                                                </button>
                                            </div>
                                        </div>
